...
class RANSAC:
    ...
    def __init__(self, n = 20, k = 1, t = None, d = 0, model = None, loss = None, metric = None):
...
def mean_absolute_error(y_true, y_pred):
    return np.sum(absolute_error_loss(y_true, y_pred)) / np.sqrt(y_true.shape[0])
...
def check(model, p, q, n, index, t):
    ...
    for i in range(n):
        ...
        vec_i[i][0] = 1
        x_i = X[i].reshape((p,1))
        temp = np.dot( inv( np.dot(XL1.T, XL1) ), XL1.T)
        a = vec_i.T - np.dot(np.dot(x_i.T, temp), MatL1)
        a = np.array(a)
...
if __name__ == "__main__":
    ...
    for i in range(trials) :
        regressor = RANSAC(model = LinearRegressionModel(), t = threshold, 
                 loss = absolute_error_loss, metric = mean_absolute_error)
        if (i + 1) % 50 == 0:
            print("Trial: ", i + 1)
        ...
        maybe_inliers = regressor.best_maybe_inliers

        rand_value = np.random.randint(len(outliers))
        j_selected = outliers[rand_value]
        p_values.append(check(regressor, num_predictor, outcome_dim, sample_size, j_selected, threshold))
