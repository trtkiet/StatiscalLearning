...
class RANSAC:
    ...
    def __init__(self, n = 50, k = None, t = None, d = 0, model = None, loss = None, metric = None):
...
def check(model, p, n, index, t, X, y):
    ...
    for inliers in accept_inliers_set:
        ...
        for i in inliers:
            ...
            vec_i[i][0] = 1
            x_i = X[i].reshape((p,1))
            a = vec_i.T - np.dot(x_i.T,np.dot(np.dot(inv(np.dot(X_Iloop.T, X_Iloop)), X_Iloop.T), MatIloop))
            s = np.dot(a,y)[0][0]
            A = np.vstack((A, -s*a))
            b = np.vstack((b, [0]))
...
if __name__ == "__main__":
    ...
    plt.hist(p_values)
    plt.savefig("./image/pvalue_with_p_predictors")
